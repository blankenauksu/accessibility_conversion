...
%TCIDATA{LastRevised=Monday, September 15, 2025 08:35:23}
...
\begin{enumerate}
\item ..Putting in resource constraints, the SP problem is%
\begin{equation*}
\underset{k}{\max }\ln \left( y_{1}-k\right) +\ln \left( k^{\alpha +\gamma
}n^{1-\alpha }\right)
\end{equation*}%
The FOC is: 
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{\left( \alpha +\gamma \right) k^{\alpha +\gamma
-1}n^{1-\alpha }}{k^{\alpha +\gamma }n^{1-\alpha }}
\end{equation*}%
or%
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{\alpha +\gamma }{k}
\end{equation*}%
$\allowbreak $Solving gives%
\begin{equation*}
k=y_{1}\frac{\alpha +\gamma }{1+\alpha +\gamma }.
\end{equation*}

\item Putting in budget constraints, the agent's problem is 
\begin{equation*}
\underset{k}{\max }\ln \left( y_{1}-k\right) +\ln \left( rk+n\omega \right)
\end{equation*}%
The FOC is 
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{r}{rk+n\omega }
\end{equation*}%
From the firm's problem 
\begin{eqnarray*}
z\alpha k^{\alpha -1}n^{1-\alpha } &=&r \\
z\left( 1-\alpha \right) k^{\alpha }n^{-\alpha } &=&\omega
\end{eqnarray*}%
or since $n=1$ 
\begin{eqnarray*}
z\alpha k^{\alpha -1} &=&r \\
z\left( 1-\alpha \right) k^{\alpha } &=&\omega
\end{eqnarray*}%
so that the FOC is 
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{z\alpha k^{\alpha -1}}{z\alpha k^{\alpha }+z\left(
1-\alpha \right) k^{\alpha }}
\end{equation*}%
or 
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{\alpha }{k}
\end{equation*}%
\begin{equation*}
k=\frac{\alpha y_{1}}{1+\alpha }
\end{equation*}

\item Now the agent's second period budget constraint is 
\begin{equation*}
rk\left( 1+\varsigma \right) +n\omega =c_{2}+\tilde{\tau}
\end{equation*}%
Then the agent's problem is 
\begin{equation*}
\underset{k}{\max }\ln \left( y_{1}-k\right) +\ln \left( rk\left(
1+\varsigma \right) +n\omega -\tilde{\tau}\right) 
\end{equation*}%
The government budget constraint 
\begin{eqnarray*}
\tau y &=&r\varsigma k \\
\tau zk^{\alpha }n^{1-\alpha } &=&\alpha zk^{\alpha -1}n^{1-\alpha
}\varsigma k \\
\tau  &=&\alpha \varsigma 
\end{eqnarray*}%
FOC for the agent's problem is 
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{r\left( 1+\varsigma \right) }{rk\left( 1+\varsigma
\right) +n\omega -\tilde{\tau}}.
\end{equation*}%
Given 
\begin{eqnarray*}
z\alpha k^{\alpha -1} &=&r \\
z\left( 1-\alpha \right) k^{\alpha } &=&\omega  \\
\tilde{\tau} &=&\tau y=zk^{\alpha }
\end{eqnarray*}%
from above and $n=1$, the FOC is%
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{z\alpha k^{\alpha -1}\left( 1+\varsigma \right) }{%
z\alpha k^{\alpha }\left( 1+\varsigma \right) +z\left( 1-\alpha \right)
k^{\alpha }-\tau zk^{\alpha }}.
\end{equation*}%
or 
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{\alpha \left( 1+\varsigma \right) }{k\left( \alpha
\left( 1+\varsigma \right) +\left( 1-\alpha \right) -\tau \right) }.
\end{equation*}%
From $\tau =\alpha s$ 
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{\alpha \left( 1+\varsigma \right) }{k\left( \alpha
\left( 1+\varsigma \right) +\left( 1-\alpha \right) -\alpha \varsigma
\right) }.
\end{equation*}%
or%
\begin{equation*}
\frac{1}{y_{1}-k}=\frac{\alpha \left( 1+\varsigma \right) }{k}.
\end{equation*}%
from which%
\begin{equation*}
k=\frac{\alpha y_{1}\left( 1+\varsigma \right) }{\alpha \left( 1+\varsigma
\right) +1}
\end{equation*}%
We want 
\begin{equation*}
\frac{\alpha y_{1}\left( 1+\varsigma \right) }{\alpha \left( 1+\varsigma
\right) +1}=\frac{y_{1}\left( \alpha +\gamma \right) }{1+\alpha +\gamma }.
\end{equation*}%
which requires 
\begin{equation*}
\varsigma =\frac{\gamma }{\alpha }
\end{equation*}
...
1-b_{i}\right) \\
\phi _{i}+\phi _{i}rb_{i} &=&\left( 1-\phi _{i}\right) \left( r-rb_{i}\right)
\\
