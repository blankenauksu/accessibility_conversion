"""Pre-upload checks for Canvas Ally accessibility on converted slide .tex files."""
...
def _split_frames(tex: str) -> list[tuple[int, str]]:
    frames: list[tuple[int, str]] = []
    for m in re.finditer(r"\\begin\{frame\}", tex):
        start = m.end()
        end_m = re.search(r"\\end\{frame\}", tex[start:])
        if not end_m:
            continue
        line = tex[: m.start()].count("\n") + 1
        frames.append((line, tex[start : start + end_m.start()]))
    return frames
...
def validate_accessible_tex(tex_path: PathLike) -> ValidationReport:
    ...
    if r"\documentclass" not in tex or "ltx-talk" not in tex:
        _add(report, "error", "not_ltx_talk", "Expected \\documentclass{ltx-talk}.")
    ...
    if not re.search(r"\\title\{[^}]+\}", tex):
        _add(report, "warning", "missing_pdf_title", "No \\title{...} in document.")
    ...
    if r"\tagpdfsetup{role/new-tag=frametitle/H1}" not in tex and "frametitle/H1" not in tex:
        _add(report, "info", "frametitle_tag", "frametitle/H1 tagging not found in \\tagpdfsetup.")
    ...
    frames = _split_frames(tex)
    if not frames:
        _add(report, "error", "no_frames", "No \\begin{frame} environments found.")
    else:
        for line, body in frames:
            loc = f"frame near line {line}"
            if not re.search(r"\\frametitle\{[^}]+\}", body) and r"\titlepage" not in body:
                _add(report, "warning", "missing_frametitle", "Frame has no \\frametitle.", loc)

            after_title = re.sub(r"\\frametitle\{[^}]*\}", "", body, count=1).lstrip()
            if re.match(r"\\item\b", after_title) and not re.match(
                r"\\begin\{(itemize|enumerate|description)\}", after_title
            ):
                _add(report, "error", "lonely_item", "Lonely \\item outside a list environment.", loc)

            open_lists = _list_stack(body)
            if open_lists:
                _add(report, "error", "unclosed_list", f"Unclosed list(s): {', '.join(open_lists)}", loc)

    if len(re.findall(r"\\begin\{frame\}", tex)) != len(re.findall(r"\\end\{frame\}", tex)):
        _add(report, "error", "frame_mismatch", "Mismatched \\begin{frame} / \\end{frame} count.")

    if report.ok and report.canvas_ready:
        stem = path.stem.removesuffix("_slides") if path.stem.endswith("_slides") else path.stem
        workdir = path.parent
        _add(
            report,
            "info",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Check slide .tex for Canvas Ally issues.")
    parser.add_argument("tex_file", help="Path to *_slides.tex")
    parser.add_argument("--strict", action="store_true", help="Exit 1 on warnings too")
    args = parser.parse_args(argv)

    report = validate_accessible_tex(args.tex_file)
